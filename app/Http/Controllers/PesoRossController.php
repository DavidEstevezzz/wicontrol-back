...
{
    /**
     * Obtiene todos los registros de peso para Ross, opcionalmente filtrados por rango de edad
     */
    public function index(Request $request)
    {
        ...
        if ($request->has('edad_inicio') && $request->has('edad_fin')) {
            ...
            return response()->json(
                PesoRoss::whereBetween('edad', [$edadInicio, $edadFin])
                    ->orderBy('edad')
                    ->get()
            );
        }
        ...
        if ($request->has('edad_inicio')) {
            ...
            return response()->json(
                PesoRoss::where('edad', '>=', $edadInicio)
                    ->orderBy('edad')
                    ->get()
            );
        }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'edad'    => 'required|integer|unique:tb_peso_ross,edad', // Asegurar que no haya duplicados
            'Mixto'   => 'required|numeric',
            'Machos'  => 'required|numeric',
            'Hembras' => 'required|numeric',
        ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'edad'    => 'sometimes|required|integer|unique:tb_peso_ross,edad,' . $item->id,
            'Mixto'   => 'sometimes|required|numeric',
            'Machos'  => 'sometimes|required|numeric',
            'Hembras' => 'sometimes|required|numeric',
        ]);
    ...
    {
        $item = PesoRoss::where('edad', $edad)->first();
    ...
    public function getPesosByRangoEdad(Request $request)
    {
        ...
        $items = PesoRoss::whereBetween('edad', [$edadInicio, $edadFin])
            ->orderBy('edad')
            ->get();
