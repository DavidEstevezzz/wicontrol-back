...
{
    /**
     * Obtiene todos los registros de peso para Cobb, opcionalmente filtrados por rango de edad
     */
    public function index(Request $request)
    {
        ...
        if ($request->has('edad_inicio') && $request->has('edad_fin')) {
            ...
            return response()->json(
                PesoCobb::whereBetween('edad', [$edadInicio, $edadFin])
                    ->orderBy('edad')
                    ->get()
            );
        }
        ...
        if ($request->has('edad_inicio')) {
            ...
            return response()->json(
                PesoCobb::where('edad', '>=', $edadInicio)
                    ->orderBy('edad')
                    ->get()
            );
        }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'edad'    => 'required|integer|unique:tb_peso_cobb,edad', // Asegurar que no haya duplicados
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'edad'    => 'sometimes|required|integer|unique:tb_peso_cobb,edad,' . $item->id,
    ...
    {
        $item = PesoCobb::where('edad', $edad)->first();
    ...
    public function getPesosByRangoEdad(Request $request)
    {
        ...
        $items = PesoCobb::whereBetween('edad', [$edadInicio, $edadFin])
            ->orderBy('edad')
            ->get();
